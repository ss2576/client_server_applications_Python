...
server_formatter = logging.Formatter(f'%(asctime)s | %(levelname)-8s | {LOGGER_NAME} | %(message)s')

stream_hnd = logging.StreamHandler(sys.stdout)
stream_hnd.setFormatter(server_formatter)
stream_hnd.setLevel(STREAM_LOG_LVL)

file_hnd = handlers.TimedRotatingFileHandler\
    (os.path.join(LOG_DIRECTORY, LOG_FILENAME),
     encoding=ENCODING, backupCount=BACKUP_COUNT,
     interval=1, when=WHEN_INTERVAL)
